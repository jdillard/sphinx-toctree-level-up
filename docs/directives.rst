...
``TocTree`` subclasses Sphinx's builtin toctree, adds ``:level-up:`` as a nonnegative int, then after ``super().run()`` writes that value onto every ``addnodes.toctree`` node.
...
.. rst:directive:: toc-level-up
   ...
   ``TocLevelUp`` stores the default on ``env.current_document`` (or ``env.temp_data`` on older Sphinx), similar to ``highlight``.
